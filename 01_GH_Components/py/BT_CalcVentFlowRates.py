...
def calcHBzoneVentRates(_HBzoneObj, _zoneGrossFloorArea):
    ventPerArea = 1
    if 'PHPProoms' in _HBzoneObj.__dict__.keys():
        tfaRoomsTotalAvgAirflow = 0
        tfaRoomsTotalFloorArea = 0
        
        for eachroom in _HBzoneObj.PHPProoms:
            avgRoomAirflow =  (eachroom.V_sup + eachroom.V_eta) / 2
            high = avgRoomAirflow * eachroom.phppVentSched.speed_high * eachroom.phppVentSched.time_high
            med = avgRoomAirflow * eachroom.phppVentSched.speed_med * eachroom.phppVentSched.time_med
            low = avgRoomAirflow * eachroom.phppVentSched.speed_low * eachroom.phppVentSched.time_low
            
            roomTotalAvgAirflow = high + med + low # The total airflow for one room
            tfaRoomsTotalAvgAirflow += roomTotalAvgAirflow # Add in the room's airflow to the runing total for the zone
            # Add in the room's Floor Area (GROSS? TFA???)
            tfaRoomsTotalFloorArea += eachroom.FloorArea_Gross
    
    # Get the Zone's total HB floor area
    zoneFloorArea = getHBzoneFloorArea(_HBzoneObj, _zoneGrossFloorArea)
    
    print "The Zone's TFA Rooms have a total average Annual airflow of: {:.3f} m3/h".format(tfaRoomsTotalAvgAirflow)
    print "    >Once you take into account the operational schedule"
    # Calc the Average Zone annual airflow
    ventPerArea = tfaRoomsTotalAvgAirflow / zoneFloorArea #tfaRoomsTotalFloorArea
    
    return ventPerArea/60/60, 0
...
if len(_HBZones) > 0:
    ...
    for zone in HBZoneObjects:
        ...
        #    (People + Area)bassed on HB Program (Load / Schedule)
        #
        ...
        ventilationPerArea_, ventilationPerPerson_ = calcHBzoneVentRates(zone, zoneGrossFloorArea_)
